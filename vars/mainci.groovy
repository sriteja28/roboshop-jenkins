...
//        }
        ...
        if(env.TAG_NAME ==~ ".*") {
            stage('Publish a Artifact') {
                sh 'docker tag ${component} 178724175855.dkr.ecr.us-east-1.amazonaws.com/${component}:${TAG_NAME}'
                sh 'aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin 178724175855.dkr.ecr.us-east-1.amazonaws.com'
                sh 'docker push 178724175855.dkr.ecr.us-east-1.amazonaws.com/${component}:${TAG_NAME}'
            }
        }
    }
}
